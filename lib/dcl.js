...
exports.default = function (_, f) {
	...
	var makeCtrStub = function makeCtrStub(ctr, advice, layerCtr) {
		if (!advice) {
			return new Proxy(ctr, { construct: function construct(_, args) {
					return new (Function.prototype.bind.apply(layerCtr, [null].concat(_toConsumableArray(args))))();
				} });
		}
		dcl._checkCtrAdvices();
		return new Proxy(ctr, { construct: function construct(target, args) {
				var i = void 0,
				    fns = advice.before,
				    result = void 0,
				    thrown = false;
				...
				result = new (Function.prototype.bind.apply(layerCtr, [null].concat(_toConsumableArray(args))))();
				fns = advice.after;
				if (fns) {
					for (i = 0; i < fns.length; ++i) {
	...
	function dcl(base) {
		...
		var layerCtr = function (_ctr) {
			_inherits(layerCtr, _ctr);

			function layerCtr() {
				_classCallCheck(this, layerCtr);

				return _possibleConstructorReturn(this, (layerCtr.__proto__ || Object.getPrototypeOf(layerCtr)).apply(this, arguments));
			}
		...
		var name = ctr.hasOwnProperty(dcl.declaredClass) && ctr[dcl.declaredClass] || Object[pname].hasOwnProperty.call(ctr[pname], dcl.declaredClass) && ctr[pname][dcl.declaredClass] || ctr.hasOwnProperty('name') && ctr.name;
		if (name && name !== 'Object') {
