...
const findCommonBase = bases => {
	...
	for (let i = 1; i < bases.length; ++i) {
		const base = bases[i];
		if (commonBase.prototype.isPrototypeOf(base.prototype)) {
			commonBase = base;
		} else if (!base.prototype.isPrototypeOf(commonBase.prototype)) {
			// incompatible bases
...
const updateDecorations = (base, action) => {
	...
	if (meta) {
		...
		remove && remove.forEach(name => delete base.prototype[name])
		meta[action] && Object.defineProperties(base.prototype, meta[action]);
	}
	return base;
};
...
function makeValueStubWithGetters (fn, advice) {
	return function () {
...
function makeValueStubDefault (fn, advice) {
	return function () {
...
function makeValueStub (fn, advice) {
...
function makeGetStub (fn, advice) {
	if (!fn) {
		fn = function () {};
	}
	const stub = function () {
...
function makeSetStub (fn, advice) {
	if (!fn) {
		fn = function () {};
	}
...
function makeCtrStub (fn, advice) {
...
function updateValue (o, name, value) {
...
function dcl(base, ...mixins) {
	...
	ctrs.forEach(mixin => {
		const ownDirectives = Object.prototype.hasOwnProperty.call(mixin, dcl.directives) && mixin[dcl.directives] ||
			Object.prototype.hasOwnProperty.call(mixin.prototype, dcl.directives) && mixin.prototype[dcl.directives];
		if (ownDirectives) {
	...
	const createDirectives = name => {
		...
		// process descriptor
		let prop = Object.getOwnPropertyDescriptor(ctr.prototype, name), newProp;
		if (!prop) {
			originalRemove.push(name);
			prop = getPropertyDescriptor(ctr.prototype, name);
		}
		if (prop) {
			original[name] = prop;
			if (prop.get || prop.set) {
				...
				if (directives.hasOwnProperty(name)) {
					...
					value = weaver(collectValues(ctr.prototype, name));
				}
				newProp = {
	...
	const name = ctr.hasOwnProperty(dcl.declaredClass) && ctr[dcl.declaredClass] ||
		ctr.prototype.hasOwnProperty(dcl.declaredClass) && ctr.prototype[dcl.declaredClass] || ctr.name;
	if (advices.constructor) {
		const prop = Object.getOwnPropertyDescriptor(ctr.prototype, 'constructor');
		original.constructor = prop;
		const advice = advices.constructor;
		advice.before && advice.before.reverse();
	...
	Object.defineProperties(ctr.prototype, reverted);
	ctr[dcl.meta] = {
